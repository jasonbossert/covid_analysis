...
ribbon_plot <- function(mcmc_draws, x, true_vals) {
  p <- c(0.025, 0.25, 0.5, 0.75, 0.975)
  p_names <- map_chr(p, ~paste0(.x*100, "%"))
  
  p_funs <- map(p, ~partial(quantile, probs = .x, na.rm = TRUE, type = 4)) %>% 
    set_names(nm = p_names)
  
  ribbon_colors <- c("95% Interval" = "lightsteelblue1", 
                     "50% Interval" = "lightsteelblue3")
  line_colors <- c("Model mean" = "lightsteelblue4", 
                   "Observed" = "black")
  
  as_tibble(t(mcmc_draws)) %>%
    mutate(time = x) %>%
    pivot_longer(cols = starts_with("V"), names_to = "sim_num", values_to = "sim_val") %>%
    mutate(sim_num = as.numeric(sub("V", "", sim_num))) %>%
    group_by(time) %>%
    summarize_at(vars(sim_val), funs(!!!p_funs)) %>%
    mutate(true = true_vals) %>%
    ggplot(aes(x = time)) +
    geom_ribbon(aes(ymin = `2.5%`, ymax = `97.5%`, fill = "95% Interval")) +
    geom_ribbon(aes(ymin = `25%`, ymax = `75%`, fill = "50% Interval")) +
    geom_line(aes(y = `50%`, color = "Model mean")) +
    geom_line(aes(y = true, color = "Observed")) +
