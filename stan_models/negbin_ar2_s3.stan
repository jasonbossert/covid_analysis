data {
  ...
  vector[1 + 5] betas_center;      // centers for beta priors
  vector<lower=0>[1 + 5] betas_scale;       // scales for beta priors
  real inv_phi_alpha;
  real inv_phi_beta;
}
...
transformed data {
  int P = 5;
  int lags = 11;
  int<lower=1> Ps[lags] = {1, 2, 7, 8, 9, 14, 15, 16, 21, 22, 23};
...
transformed parameters {
  vector[1 + lags] betas_ext = [betas[1], betas[2], betas[3], 
                                betas[4], -1 * betas[2] * betas[4], -1 * betas[3] * betas[4],
                                betas[5], -1 * betas[2] * betas[5], -1 * betas[3] * betas[5],
                                betas[6], -1 * betas[2] * betas[6], -1 * betas[3] * betas[6]]';
  vector[T] nu;
